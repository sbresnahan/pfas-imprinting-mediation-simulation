...
sem_table <- data.frame(
  ...
  Pattern = c(
    ...
    "Roughly uniform",
    "Extreme left skew (nearly all p < 0.05)",
    "Right skew (most p > 0.5)",
    "Right skew (strongest around 1.0)",
  ...
  Interpretation = c(
    ...
    "PFAS has a direct effect on SRS in nearly all isoforms, suggesting true mediation.",
    "Female-specific indirect effects are weak or absent across most isoforms.",
    "Male-specific mediation effects appear sparse and mostly non-significant.",
...
coca_pgc_table <- data.frame(
  Method = c("COCA", "PGC"),
  PValue_Pattern = c(
    "All p-values near zero for PFAS, sex, and interaction — suggesting inflated Type I error",
    "PFAS p-values are modestly left-skewed; sex and interaction terms are strongly right-skewed"
  ),
  CI_Significance = c(
    "100% of isoforms significant for PFAS; 0% for sex or interaction",
    "~10% of isoforms significant for PFAS; 0% for sex or interaction"
  ),
  Interpretation = c(
    "COCA falsely detects PFAS effects for all isoforms, revealing extreme susceptibility to residual confounding and misspecification.",
    "PGC partially detects true PFAS mediation but still inflates effect detection under simulated confounding; improves specificity for sex and interaction terms."
  )
)
...
SEM moderately inflates Type I error for the individual path coefficients suggesting over-sensitivity to noise in component paths of the mediation model under the null. However, the Type I error for the indirect effects (0.08 for females and 0.06 for males) is much closer to nominal levels, indicating that the bootstrapped product-term approach provides some protection against false positives for mediated effects, even when component paths are overestimated.

##### COCA

COCA displays severe inflation of Type I error for the PFAS main effect (1.00), despite being under a null simulation, implying a total failure to control false positives. The fact that fetal sex and interaction terms remain controlled (0.00) underscores that the bias is specific to PFAS and likely arises from residual confounding in the unadjusted PFAS -> methylation path. This confirms the method’s vulnerability in the presence of latent confounders.

##### PGC

PGC substantially inflates Type I error for the PFAS effect (0.83), though not as dramatically as COCA. The sex and interaction terms remain well-controlled (0.00), suggesting that PGC’s use of expression as a proxy partially mitigates bias but does not eliminate it. The high PFAS false positive rate indicates that under partial confounding, even bridge-variable strategies can produce misleading results unless the proxy is very strong and correctly specified.

##### Direct Adjustment

Direct adjustment (e.g., SRS ~ expression + sex + interaction) provides the most reliable Type I error control among all methods. The error rates for expression (0.08), sex (0.02), and interaction (0.02) remain close to or below nominal levels, indicating conservative performance. While this method doesn't model mediation explicitly, it performs robustly under the null and may serve as a useful baseline in exploratory or confirmatory settings.