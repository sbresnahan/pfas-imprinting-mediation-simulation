...
for (i in mediated_isoforms) {
  effect <- scale(PFAS) * rnorm(1, mean = 0.05, sd = 0.01)
...
coca_pgc_table <- data.frame(
  Method = c("COCA", "PGC"),
  PValue_Pattern = c(
    "Nearly all p-values > 0.9 for PFAS and interaction; sex effect shows spurious signal",
    "PFAS p-values strongly left-skewed; interaction and sex terms flat"
  ),
  CI_Significance = c(
    "0 isoforms significant for PFAS or PFAS×sex; 100 isoforms for fetal_sex (false positive)",
    "97 isoforms significant for PFAS (97%); 0 for sex or interaction"
  ),
  Interpretation = c(
    "COCA fails to detect true mediation effects and produces false positives for fetal sex, consistent with its known instability when proxy variables are imperfect or interact with exposures.",
